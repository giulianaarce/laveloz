...
export default class ProductoDetalleFav extends React.Component{
    constructor(){
        super()
        this.state = {
            productos: "",
    ...
    componentDidMount() {
        ...
        fetch(`http://localhost:4200/api/sugeridos/${this.props.producto_id}`)
            .then((res) => { return res.json() })
            .then((json) => {
                this.setState({ productos: json })
                console.log("ProductoFav",this.state.productos)
            })

        fetch(`http://localhost:4200/api/mercadofav/${this.props.producto_id}`)
            .then((res) => { return res.json() })
            .then((json) => {
